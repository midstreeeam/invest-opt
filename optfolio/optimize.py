...
class Optimizer:
    ...
    def run(self, returns: np.ndarray, max_allocation: float = None) -> Tuple(np.ndarray, dict):
        ...
        if max_allocation is not None:
            constraints_val += max_allocation_constraint(
                population, max_allocation)
        fronts, crowding_distances = non_dominated_fronts(
            return_obj, volatility_obj, constraints_val)
        ...
        for gen_idx in range(self._max_iter):
            ...
            for i in range(self._population_size):
                (p1_idx, p2_idx) = tournament_selection(
                    fronts, crowding_distances)
                offspring[i, :] = flat_crossover(
                    population[p1_idx], population[p2_idx])
                if np.random.uniform() < mutation_p:
                    offspring[i, :] = gaussian_mutation(
                        offspring[i, :], sigma=self._mutation_sigma)
            ...
            if self._verbose:
                print('Generation: %d, Time: %.2f, Return: %.2f, Volatility: %.2f, Constraints violation: %.4f, N pareto solutions: %d' % (
                    gen_idx, stats['time_per_generation'][-1], stats['return']['avg'][-1] *
                    100, stats['volatility']['avg'][-1] *
                    100, stats['constraints_violation']['max'][-1], np.sum(
                        fronts == 0)
                ))
